...
import type { Product, Category, CartItem, Frosting, Order } from "@/lib/types";
...
export default function BarPage() {
  ...
  async function updateOrderStatus(orderId: string, status: "ready" | "delivered") {
    try {
      ...
      if (!res.ok) throw new Error();
      toast.success(
        status === "ready" ? "Orden lista" : "Orden entregada"
      );
      fetchOrders();
    } catch {
  ...
  return (
    <div className="flex h-screen">
      {/* LEFT PANEL (70%): Take Orders + Cart */}
      ...
      {/* RIGHT PANEL (30%): Dispatch Orders */}
                        ...
                        <div className="space-y-1 text-xs">
                          {order.items?.map((item) => (
                            <div key={item.id}>
                              ...
                              {item.frostingName && (
                                <span className="text-blue-600"> ({item.frostingName})</span>
                              )}
                            </div>
                          ))}
                        </div>
